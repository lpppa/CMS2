...
<body>
<div>
    	<form action="${pageContext.request.contextPath }/articleWrite?userAccountId=${currentUser.userId }" method="post" enctype="multipart/form-data">
			<input type="text" name="articleTitle" placeholder="请输入新闻名称" />
			<input type="file" name="articlepicture"/>
			<select name="aricleTypeId">
				<option value="0">请选择要发布的新闻类别</option>
				<c:forEach var="articleType" items="${articleTypeList }">
					<option value="${articleType.articleTypeId }">${articleType.articleTypeName }</option>
				</c:forEach>
			</select>
	  		<script id="editor" type="text/plain" style="width:1024px;height:500px;" name="articleContent"></script>
